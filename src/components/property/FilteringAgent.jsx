...
export default function FilteringAgent() {
  ...
  useEffect(() => {
    ...
    let filteredArrays = [...refItems];

    if (propertyTypes.length > 0) {
      const filtered = filteredArrays.filter((elm) =>
        propertyTypes.includes(elm.category)
      );
      filteredArrays = [filtered];
    }
    if (searchQuery)
      filteredArrays = [
        ...filteredArrays.filter((el) =>
          el.name.toLocaleLowerCase().includes(searchQuery.toLocaleLowerCase())
        ),
      ];

    if (location != "All Cities") {
      filteredArrays = [...filteredArrays.filter((el) => el.city == location)];
    }

    // const commonItems = refItems.filter((item) =>
    //   filteredArrays.every((array) => array.includes(item))
    // );
    setFilteredData(filteredArrays);
