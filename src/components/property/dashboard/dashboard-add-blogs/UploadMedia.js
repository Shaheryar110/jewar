...
import { imageUpload, imageUploadBlog } from "@/services/ImageUpload";

const UploadMedia = ({ handleChange, currentRtl }) => {
  ...
  const handleUpload = (e) => {
    ...
    if (e.target.files.length > 0) {
      ...
      imageUploadBlog(e.target.files[0])
        .then((data) => {
          newImages.push(data);
    ...
    <>
      <div className="upload-img position-relative overflow-hidden bdrs12 text-center mb30 px-2">
        ...
        <label className="ud-btn btn-white">
          {currentRtl === "ltr" ? "Browse Files" : "ملف الاستعراض"}
